...
**Two-pane layout** (`features/review-panel/components/ReviewLayout.tsx`): a `react-resizable-panels` PanelGroup with a `<Sidebar>` (collapsible, file filter + tree/flat view, file list via `@pierre/trees`) and a `<DiffPane>` (toolbar + status bar + `@pierre/diffs` PatchDiff per file). Keyboard shortcuts (`j`/`k`/`gg`/`G`/`/`/`[`) are wired in `hooks/use-keyboard-shortcuts.ts`. File navigation state is centralized in `hooks/use-file-nav.ts`.
...
- Token taxonomy in `panda.config.ts`:
  - **Raw**: `colors.neutral.{50…950}`, `colors.diff.{addLight,addDark,delLight,delDark}`, `spacing.{1, 1.5, 2, 2.5, 3, 3.5, 4}` (4–16px), `sizes.control.{xs, sm, md, lg, xl}` (24–36px), `radii.{sm, md}`, `fontSizes.{xs, sm, md, lg}` (11–14px), `fonts.{sans, mono}`, `easings.standard`.
  - **Semantic** (resolves via `_dark` condition): `colors.bg.{canvas, surface, hover, subtle, selected}`, `colors.fg.{default, muted, onSelected}`, `colors.border.{default, strong}`, `colors.focus.ring`, `colors.diff.{add, del}`. Always prefer semantic colors; raw `neutral.*` is for token definitions only.
...
**`theme/tokens.ts` is intentionally retained** despite duplicating values in `panda.config.ts`. `FileNav.tsx` passes raw color values (`bg.surface`, `fg.default`, `diff.add/del`) into `@pierre/trees`'s `themeToTreeStyles` and CSS variable injection — those need JS-runtime strings, not CSS-var references. Keep the two in sync when changing palette values.
