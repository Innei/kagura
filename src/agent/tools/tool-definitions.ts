...
} from '../providers/claude-code/tools/recall-memory.js';
import {
  parseSaveMemoryToolInput,
  SAVE_MEMORY_TOOL_DESCRIPTION,
  SAVE_MEMORY_TOOL_NAME,
} from '../providers/claude-code/tools/save-memory.js';
import {
...
} from '../providers/claude-code/tools/set-channel-default-workspace.js';
import {
  parseUploadSlackFileToolInput,
  UPLOAD_SLACK_FILE_TOOL_DESCRIPTION,
  UPLOAD_SLACK_FILE_TOOL_NAME,
} from '../providers/claude-code/tools/upload-slack-file.js';
import {
  RecallMemoryToolInputSchema,
  SaveMemoryToolInputSchema,
} from '../providers/claude-code/schemas/memory-tools.js';
import { SetChannelDefaultWorkspaceToolInputSchema } from '../providers/claude-code/schemas/channel-preference-tools.js';
import { UploadSlackFileToolInputSchema } from '../providers/claude-code/schemas/upload-slack-file.js';
import type { AgentToolDefinition } from './types.js';

export interface ToolDefinitionDependencies {
...
export function createToolDefinitions(
  deps: ToolDefinitionDependencies,
): AgentToolDefinition[] {
  return [
    createRecallMemoryTool(deps),
    createSaveMemoryTool(deps),
    createSetChannelDefaultWorkspaceTool(deps),
    createUploadSlackFileTool(deps),
  ];
}

function createRecallMemoryTool(deps: ToolDefinitionDependencies): AgentToolDefinition {
  const { logger, memoryStore, request } = deps;
  return {
    name: RECALL_MEMORY_TOOL_NAME,
    description: RECALL_MEMORY_TOOL_DESCRIPTION,
    inputSchema: RecallMemoryToolInputSchema,
    execute: async (args) => {
      try {
...
function createSaveMemoryTool(deps: ToolDefinitionDependencies): AgentToolDefinition {
  const { logger, memoryStore, request } = deps;
  return {
...
function createSetChannelDefaultWorkspaceTool(
  ...
  const { channelPreferenceStore, logger, request } = deps;
  return {
    name: SET_CHANNEL_DEFAULT_WORKSPACE_TOOL_NAME,
    description: SET_CHANNEL_DEFAULT_WORKSPACE_TOOL_DESCRIPTION,
    inputSchema: SetChannelDefaultWorkspaceToolInputSchema,
    execute: async (args) => {
      try {
...
function createUploadSlackFileTool(deps: ToolDefinitionDependencies): AgentToolDefinition {
  const { logger, request, sink } = deps;
  return {
    name: UPLOAD_SLACK_FILE_TOOL_NAME,
    description: UPLOAD_SLACK_FILE_TOOL_DESCRIPTION,
    inputSchema: UploadSlackFileToolInputSchema,
    execute: async (args) => {
      try {
...
): { missingWorkspace: boolean; repoId: string | undefined; scope: MemoryScope } {
